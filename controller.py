...
@app.route("/historico_calculadora_adicionar", methods=["GET","POST"])
def historico_calculadora_adicionar():
    if 'usuario_logado' not in session or session['usuario_logado'] == None:
        return render_template("tela_login.html")
    data = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    usuario = str(session['nome'])
    mdo_hr_preparacao = request.args.get('mdo_hr_preparacao')
    mdo_valor_preparacao = request.args.get('mdo_valor_preparacao')
    mdo_hr_costura = request.args.get('mdo_hr_costura')
    mdo_valor_costura = request.args.get('mdo_valor_costura')
    mdo_hr_estofamento = request.args.get('mdo_hr_estofamento')
    mdo_valor_estofamento = request.args.get('mdo_valor_estofamento')
    mdo_valor_geral = request.args.get('mdo_valor_geral')
    mdo_percentual_hr = request.args.get('mdo_percentual_hr')
    mdo_total_valor = request.args.get('mdo_total_valor')
    tipo_tecido = request.args.get('tipo_tecido')
    qtd_tecido = request.args.get('qtd_tecido')
    valor_estofaria_tecido = request.args.get('valor_estofaria_tecido')
    valor_cliente_tecido = request.args.get('valor_cliente_tecido')
    qtd_espuma_1 = request.args.get('qtd_espuma_1')
    altura_espuma_1 = request.args.get('altura_espuma_1')
    largura_espuma_1 = request.args.get('largura_espuma_1')
    comprimento_espuma_1 = request.args.get('comprimento_espuma_1')
    valor_estofaria_espuma_1 = request.args.get('valor_estofaria_espuma_1')
    valor_estofaria_cliente_1 = request.args.get('valor_estofaria_cliente_1')
    qtd_espuma_2 = request.args.get('qtd_espuma_2')
    altura_espuma_2 = request.args.get('altura_espuma_2')
    largura_espuma_2 = request.args.get('largura_espuma_2')
